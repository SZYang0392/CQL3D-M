...
ifeq ($(os),Linux)
	...
	ifneq (,$(filter $(strip $(hostname)),login110 service106))
	...
	PGPLOT_HOME ?= $(or $(PGPLOT_DIR),$(EBROOTPGPLOT))
    ...
    # 检测X11库是否存在
	HAVE_X11 := $(shell ldconfig -p | grep -q libX11.so && echo yes)

    # 包含路径
	LOCATION := -L/user/lib64 -L$(PGPLOT_HOME) -L$(NETCDF_C_HOME)/lib -L$(NETCDF_FORTRAN_HOME)/lib
	LIBS := $(if $(HAVE_X11), -lX11) -lnetcdff -lnetcdf -lpgplot
	INCLUDE := -I/usr/include/ -I$(PGPLOT_HOME)/ -I$(NETCDF_C_HOME)/include/ -I$(NETCDF_FORTRAN_HOME)/include/
endif
...
#DEBUG     = -g -Mbounds
DEBUG     = -g 
OPTIMIZE  = -O -fno-align-commons
LISTING   = -Mlist
CSPECIAL  = -Wno-padded
#SPECIAL   = -byteswapio -Mbackslash
LDSPECIAL = -Wl,-noinhibit-exec 
#COMPILE   = mpif77 -c $(CSPECIAL) -c $(DEBUG) $(INCLUDE) # or use $(OPTIMIZE)
COMPILE   = mpif77 -c $(CSPECIAL) $(OPTIMIZE) $(INCLUDE) # or use $(DEBUG)
